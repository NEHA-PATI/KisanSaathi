...
const SupplierDashboard = () => {
    ...
    const loadSupplierDashboard = async () => {
      try {
        ...
        setDashboardState({
          ...
          operatingAreas: Array.isArray(data?.operatingAreas) ? data.operatingAreas : [],
        });
      } catch (error) {
        if (!isMounted) return;
        ...
        setDashboardState({
          loading: false,
          error: error.message || "Unable to load supplier dashboard data right now.",
          traces: [],
          operatingAreas: [],
        });
      }
  ...
  const filteredTraces = useMemo(
    ...
    [normalizedQuery, statusFilter, traces],
  );

  const availableCount = traces.filter((trace) => !trace.assignedPatchId).length;
  const batchedCount = traces.length - availableCount;
  const uniqueGrowers = new Set(traces.map((trace) => trace.growerUserId).filter(Boolean)).size;
  const totalWeight = traces.reduce((sum, trace) => sum + (Number(trace.netWeight) || 0), 0);
  const areaLabels = dashboardState.operatingAreas
    .map((area) =>
      [area.village, area.district, area.state, area.pincode].filter(Boolean).join(", "),
    )
    .filter(Boolean);
        ...
        <div className="supplier-dashboard-copy">
          ...
          <div className="supplier-dashboard-actions">
            <div className="supplier-dashboard-note">
              Live data is matched against supplier registration locations and farmer farm records.
            </div>
          </div>
        </div>
            ...
            return (
              <article key={trace.packingId} className="supplier-trace-card-dashboard">
                <div className="supplier-trace-header">
                  ...
                  <span className={`supplier-trace-status-badge ${isAvailable ? "available" : "batched"}`}>
                ...
                <div className="supplier-trace-location">
                  ...
                  {trace.matchedArea && <div className="supplier-match-pill">Matched area: {trace.matchedArea}</div>}
                </div>
